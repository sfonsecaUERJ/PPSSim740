...
*/
#include "SimRomanPot/PPSSim/interface/PPSConstants.h"
//=====================================================================================================
...
public:
       ...
       void set_TrackerPosition(double p)   {fTrackerPosition=p;};
       ...
       void set_ToFPosition(double p)       {fToFPosition=p;};
       void set_TCLPosition(const string& tcl,double z1,double z2) {
                                                         ...
                                                         else std::cout <<"WARNING: Unknown Collimator " << tcl << endl;
                                            }
       ...
       void add_OutgoingParticle(int ivtx, const TLorentzVector* pF,const TLorentzVector* pB)
                     {if (ivtx>=NVertex) {std::cout<<"Error: Adding particle to a non existing vertex."<<std::endl;}
       ...
       void Simulation();
       void Reconstruction();
       bool SearchTrack(int i,int j,int Direction,H_RecRPObject* station,PPSData* arm, int& stationId,double& xi,double& t,double& partP,double& pt,double& phi,double& theta,double& ImpPar);
       void TrackerReco(int Direction,H_RecRPObject* station,PPSData* arm);
       void ToFReco();
       void Digitization();
       void TrackerDigi(const PPSData*,PPSData*);
       void ToFDigi(const PPSData*,PPSData*);
       void ReconstructArm(int Direction, H_RecRPObject* pps_station, PPSData* arm, int idx=0);
       void ReconstructArm(H_RecRPObject* pps_station, double x1,double y1,double x2,double y2, double& tx, double& ty,double& eloss);
       void TLorentzVectorToPPSData(PPSData*,const TLorentzVector*);
       ...
       PPSSpectrometer* get_GenDataHolder() {return fGen;};
       PPSSpectrometer* get_SimDataHolder() {return fSim;};
       PPSSpectrometer* get_RecoDataHolder(){return fReco;};
